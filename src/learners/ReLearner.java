...
public class ReLearner {   
    ...
    List <String>charStateDelims = new ArrayList<String>();
    ...
    public boolean addCharacterExample(String line, int startCharPos, int endCharPos)
    {
    	...
    	{
    		if (chr.matches(".*\\w.*") && !charStateDelims.contains(".") )
    		{
    			charStateDelims.add(".");
    			return true;
    		}
    	...
    	if (!charStateDelims.contains(charStateDelimiter))
    	{
    		charStateDelims.add(charStateDelimiter);
    	}
	...
	else if(AlgoType.REGEX_TEMPLATE == learningAlgorithm)
	{
		...
		System.out.println ("Newly learned delims are : ");
		for (int i=0;i<charStateDelims.size();i++)
		{
			System.out.println(charStateDelims.get(i));
		}
        ...
        public Vector<Pairs> getCharacterAndStatesUsingRegexTemplatesPair()
	{
			...
			String regexDelims = "";
			for (String s:charStateDelims)
			{
				regexDelims += s;
			}
			String[] tokens = dataRows.elementAt(i).split(regexDelims);
			String curCharacter = "";
...
//		                System.out.println("I found the text - " + matcher.group() + " starting at " +"index " + matcher.start() +" and ending at index " + matcher.end());
    ...
    public Vector<Pairs>  getCharacterAndStatesUsingRegexTemplates()
	{
		...
		for (int chrs=0;chrs<charStateDelims.size();chrs++)
		{
			String txt = charStateDelims.get(chrs);
			if (txt.equalsIgnoreCase(","))
				continue;
			
			if(curLine.contains(txt))
			{
				boolean char_over = false;
				String[] tokens = dataRows.elementAt(i).split(txt);
				if (tokens.length == 0 && !curLine.contains(":"))
				{
					tokens = dataRows.elementAt(i).split("\\.");
					if (tokens[0].matches("\\d"))
					{
						String []temp = new String[1];
						temp[0] = tokens[0]+". "+tokens[1];
						tokens = temp;
					}
				}
				String curCharacter = "";
			...
			for(int j=0;j<tokens.length-0;j++)
			{
				...
				if(char_over) combinedStates += charStateDelims.get(chrs) + tokens[j];
				else {	
						
						if(j>0) curCharacter+=charStateDelims.get(chrs);
						curCharacter += tokens[j];
				}
			}
		  ...
		  regexMatchedStates.add(curre);
			...
			Pairs p = new Pairs();
                        ...
                        else
                        {
                        	if (curCharacter !="")
                        	{
                        		String []temp = curCharacter.split("\\.");
                        		if (temp != null)
                        		{
                        			if (temp[0].matches("[0-9]+")) {
                        				if (temp[1] != null) {
                        					String out_char = temp[0] + "."+temp[1];
                        					System.out.println(out_char);
                        					curCharacter = out_char;
                        				}
                        			}
                        		}
