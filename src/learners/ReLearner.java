...
public class ReLearner {
	...
	public ReLearner(Vector<String> dr, AlgoType alg){
		...
		charDelimiters = new Vector<String>();
	...
	public boolean addCharacterExample(String line, int startCharPos, int endCharPos)
	{
		...
		{
			charDelimiters.add(charStateDelimiter.trim());
		}
		else
		{
			if (!charDelimiters.contains(charStateDelimiter.trim()))
				charDelimiters.add(charStateDelimiter.trim());
			
			if (charDelimiters.elementAt(0).equalsIgnoreCase("."))
			{
				Collections.swap(charDelimiters, 0, charDelimiters.size()-1);
			}
		}
...
isExistingTemplate = true;
	...
	public Vector<Pairs> getCharacterAndStatesUsingRegexTemplates()
	{
		...
		for(int i=0;i<dataRows.size();i++)
		{
			...
			{
				String temp = "";
				temp +=curLine.charAt(j);
				if (!temp.matches("\\d"))
				{
					if (j+1<curLine.length())
					{
						startpos = j+1;
						break;
					}
			...
			testLine = curLine.substring(startpos,curLine.length()).trim();
			for (int delim=0;delim < charDelimiters.size();delim++)
			{
				charStateDelimiter = charDelimiters.elementAt(delim);
				if(testLine.contains(charStateDelimiter))
				{
					...
					for(int j=0;j<tokens.length-0;j++)
					{
						if(tokens[j] != null)
						{
							...
							if (tokens[j].matches("[0-9]+\\.\\s[\\w\\s]*"))
							{
								if (curCharacter.equalsIgnoreCase(""))
									curCharacter = tokens[j];
							}
							else {
								if(char_over) {
									    ...
									    if ((tokens[j].contains("“") || tokens[j].contains("(")) && (curCharacter.equalsIgnoreCase("")))
									    {
									    	...
									    	combinedStates += charStateDelimiter + tokens[j];
								}
								else {
									if (curCharacter.equalsIgnoreCase(""))
										curCharacter = tokens[j];
					...
					//if(curStates.size() == statestokens.length) /* Line is not very clear */
					{
						...
						{
							if (curCharacter!="" && charStateDelimiter.equalsIgnoreCase("."))
							{
